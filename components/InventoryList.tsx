...
const InventoryList: React.FC = () => {
  const columns = useMemo<ColumnDef<any>[]>(() => [
    ...
    },
    {
      accessorKey: 'type',
      header: 'Category',
      cell: info => <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{String(info.getValue())}</span>,
    },
    {
      accessorKey: 'stock',
      header: 'Availability',
      cell: info => {
        ...
        return (
          <div className="flex items-center space-x-2">
            <span className={`font-bold ${stock < 10 ? 'text-red-500' : 'text-slate-900'}`}>{stock} Units</span>
    ...
    },
    {
      accessorKey: 'price',
      header: 'Unit Price',
      cell: info => <span className="font-bold text-slate-900">₦{Number(info.getValue()).toLocaleString()}</span>,
    },
    {
      id: 'actions',
      header: 'Actions',
      cell: () => (
        <DropdownMenu 
          trigger={<button className="p-2 hover:bg-gold hover:text-slate-900 rounded-lg transition-colors bg-slate-50 border border-slate-200">Update</button>}
          items={[
    ...
    <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">Monitor stock levels across all categories.</p>
        </div>
      </div>
