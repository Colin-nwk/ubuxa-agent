...
const ColorSwatch: React.FC<{ name: string, hex: string, cssClass: string }> = ({ name, hex, cssClass }) => (
  <div className="space-y-2 group">
    <div className={`h-16 rounded-2xl border border-slate-200 ${cssClass} shadow-sm group-hover:scale-105 transition-transform duration-300`} />
    <div>
...
const ComponentGallery: React.FC = () => {
  ...
  const demoColumns = useMemo<ColumnDef<DemoData>[]>(() => [
    ...
    },
    {
      accessorKey: 'amount',
      header: 'Amount',
      cell: info => <span className="text-gold font-bold">₦{(info.getValue() as number).toLocaleString()}</span>,
    },
    {
      accessorKey: 'status',
      header: 'Status',
    ...
    },
    {
      ...
      cell: () => (
        <DropdownMenu 
          trigger={<button className="p-2 hover:bg-slate-100 rounded-lg"><MoreVertical size={16} /></button>}
          items={[
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-700 pb-20">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h2 className="text-3xl font-serif font-bold text-slate-900">UI Library</h2>
          <p className="text-slate-500 mt-1 uppercase text-[10px] font-bold tracking-[0.2em]">UBUXA Design System v1.4</p>
        </div>
        <div className="flex items-center space-x-2 bg-slate-100 p-1.5 rounded-2xl">
           <span className="px-3 py-1.5 bg-white shadow-sm rounded-xl text-xs font-bold text-slate-900">Component Guide</span>
      ...
      <section className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 bg-white p-8 rounded-[2.5rem] border border-slate-100">
          <ColorSwatch name="Slate 900" hex="#0f172a" cssClass="bg-slate-900" />
          <ColorSwatch name="Gold Core" hex="#d4af37" cssClass="bg-[#d4af37]" />
          <ColorSwatch name="Gold Light" hex="#f1d592" cssClass="bg-[#f1d592]" />
          <ColorSwatch name="Success Green" hex="#16a34a" cssClass="bg-green-600" />
          <ColorSwatch name="Danger Red" hex="#dc2626" cssClass="bg-red-600" />
          <ColorSwatch name="Surface" hex="#f8fafc" cssClass="bg-slate-50" />
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <PrimaryButton className="!bg-green-600 !shadow-green-200" onClick={() => openModal('md', 'success')}>Success</PrimaryButton>
                <PrimaryButton className="!bg-red-600 !shadow-red-200" onClick={() => openModal('md', 'danger')}>Danger</PrimaryButton>
                <PrimaryButton className="!bg-gold !shadow-gold/20" onClick={() => openModal('md', 'warning')}>Warning</PrimaryButton>
              </div>
            </div>
            <div className="space-y-4">
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest pl-1">Responsive Sizes</p>
              <div className="flex flex-wrap gap-3">
                ...
                <SecondaryButton onClick={() => openModal('xl', 'default')}>X-Large</SecondaryButton>
                <SecondaryButton onClick={() => openModal('full', 'default')}>Full Screen</SecondaryButton>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-4">
                <button className="w-12 h-12 bg-slate-900 text-gold rounded-2xl flex items-center justify-center shadow-lg hover:scale-110 transition-transform">
                  <Plus size={24} />
                </button>
                <button className="w-12 h-12 bg-gold-gradient text-slate-900 rounded-2xl flex items-center justify-center shadow-lg hover:scale-110 transition-transform">
                  <CheckCircle2 size={24} />
                </button>
                <button className="w-12 h-12 bg-white border border-slate-200 text-slate-400 rounded-2xl flex items-center justify-center shadow-sm hover:border-gold hover:text-gold transition-all">
                  <Settings size={20} />
                </button>
              </div>
            </div>
      ...
      <section className="space-y-6">
        ...
        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 space-y-12">
          {/* Text Inputs */}
          ...
          <div className="pt-8 border-t border-slate-50 grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div className="space-y-8">
          <div className="bg-white p-2 rounded-[3.5rem] border border-slate-100 shadow-sm overflow-hidden">
             <div className="p-8 pb-4">
                <h4 className="text-xl font-serif font-bold text-slate-900">Responsive Data Table</h4>
                <p className="text-xs text-slate-500 mt-1 uppercase tracking-wider font-bold">Integrated with TanStack Table</p>
             </div>
             <div className="p-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-4">
              ...
              <div className="p-12 bg-slate-900 rounded-[2.5rem] text-center text-white border-4 border-gold/10">
                 <p className="text-sm opacity-60">Displaying content for</p>
                 <h5 className="text-2xl font-serif font-bold mt-1 text-gold">{activeTab.toUpperCase()}</h5>
              </div>
            </div>
            <div className="space-y-4">
      ...
      <section className="space-y-6">
        ...
        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100">
           ...
           <div className="mt-6 flex items-center space-x-3">
              ...
              <p className="text-sm text-slate-600 font-medium italic">Integrated mapping interface used for customer coordinate verification during installation audits.</p>
           </div>
        </div>
      </section>
      ...
      >
        <div className="space-y-4">
          ...
          <div className="p-5 bg-slate-50 rounded-2xl border border-slate-200">
            <p className="text-xs font-bold text-slate-900 mb-2">Technical Specs:</p>
      ...
      >
        <div className="space-y-6">
           <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100 flex items-center space-x-4">
              ...
              <div>
                <h4 className="font-bold text-slate-900">Drawer Workflow</h4>
                <p className="text-xs text-slate-500">Ideal for data entry that requires reference to the main view.</p>
              </div>
           </div>
      ...
      <div className="fixed bottom-24 right-8 z-[80] hidden lg:block">
        <button 
          ...
          className="w-16 h-16 bg-slate-900 text-gold rounded-full flex items-center justify-center shadow-2xl hover:scale-110 transition-transform group relative"
        >
          <Layers size={28} />
          <div className="absolute right-full mr-4 top-1/2 -translate-y-1/2 bg-slate-900 text-white px-3 py-1.5 rounded-xl text-[10px] font-bold opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap shadow-xl">
             Try Side Drawer
